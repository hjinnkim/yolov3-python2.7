...
def evaluate_model_file(model_path, weights_path, img_path, class_names, batch_size=8, img_size=416,
    ...
    :param conf_thres: Object confidence threshold, defaults to 0.1
...
def print_eval_stats(metrics_output, class_names, verbose):
    if metrics_output is not None:
        ...
        print("---- Positive True mAP {:.5f} ----".format(AP.mean()))
    else:
...
def run():
    ...
    parser.add_argument("--weights", type=str, default="weights/yolov3-tiny.weights", help="Path to checkpoint file (.weights or .pth). Starts training from checkpoint model")

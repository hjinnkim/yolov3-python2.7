...
def detect_directory(model_path, weights_path, img_path, classes, output_path,
    ...
    print("---- Detections were saved to: '{}' ----".format(output_path))
...
def _draw_and_save_output_image(image_path, detections, img_size, output_path, classes):
    ...
    n_class = len(classes)
    classes.append("wrong_detecting")

    for x1, y1, x2, y2, conf, cls_pred in detections:
        print(x1, y1, x2, y2, conf, cls_pred)

        idx_cls_pred = n_class if int(cls_pred) >= n_class else int(cls_pred)

        print("\t+ Label: {} | Confidence: {:0.4f}".format(classes[idx_cls_pred], conf.item()))

        box_w = x2 - x1
        box_h = y2 - y1

        color = bbox_colors[int(np.where(unique_labels == cls_pred)[0])]
    ...
    output_path = os.path.join(output_path, "{}.png".format(filename))
...
def run():
    ...
    parser.add_argument("-c", "--classes", type=str, default="data/custom.names", help="Path to classes label file (.names)")
    parser.add_argument("-o", "--output", type=str, default="data/output", help="Path to output directory")

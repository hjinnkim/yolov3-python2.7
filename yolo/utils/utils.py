...
def result_visualize(loss_list, lr_list, mAP_list):

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax2 = ax.twinx()
    ax.plot(loss_list, '-', label = 'loss')
    ax.plot(mAP_list, '-', label = 'Positive true mAP')

    ax2.plot(lr_list, '-r')  # The true line in ax2
    ax.plot(np.nan, '-r', label = 'learning rate')  # Make an agent in ax
    ...
    plt.savefig("train_results/train_result.png")
    print("---- Training result was saved to: '{}' ----".format("train_results/train_result.png"))